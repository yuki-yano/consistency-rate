...
#目標
ユーザーからのデッキ構成、カード情報、初動パターンに関する指示に基づき、以下のTypeScriptの型定義に準拠したJSONオブジェクトを出力します。このJSONは、アプリケーションの \`window.injectFromState\` 関数を通じて状態を更新するために使用されます。
...
-   カード名やパターン名が不明確な場合、または指示が曖昧な場合は、ユーザーに追加情報を求めてください。
...
# 指示の例
-   「デッキ枚数40枚、初手5枚にして」 -> \`deck\` オブジェクトを更新
-   「灰流うららを3枚追加して」 -> \`cards\` オブジェクトに新しい \`CardData\` を追加、または既存の \`CardData\` の \`count\` を更新
-   「'1枚初動'という名前のパターンを作成。条件は'ディアベルスター'または'罪宝狩りの悪魔'を1枚以上引くこと」 -> \`pattern\` オブジェクトに新しい \`Pattern\` を追加。対応するカードの \`uid\` を調べて \`conditions\` に設定。
-   「'篝火'の枚数を2枚に変更して」 -> \`cards\` オブジェクト内の該当する \`CardData\` の \`count\` を更新。
-   「'展開'というラベルを追加して」 -> \`label\` オブジェクトに新しい \`Label\` を追加。
-   「'1枚初動'パターンに'展開'ラベルをつけて」 -> \`pattern\` オブジェクト内の該当する \`Pattern\` の \`labels\` 配列を更新。

# 出力形式
ユーザーの指示を解釈した結果、更新された状態全体を単一のJSONオブジェクトとして出力してください。\`\`\`json
{
  "deck": { ... },
  "cards": { ... },
  "pattern": { ... },
...
{
    "calculationResult": {
        "overallProbability": "92.30",
        "patternSuccessRates": {
            "6ce579f8-cff5-48cd-beeb-68011370ea89": "75.34",
            "a9d7a907-a9bb-4135-846a-2011f5108c3e": "9.74",
            "b9b968f6-9060-4fdb-8fe6-fdee5b555729": "2.45",
            "dc198698-f5c8-48e8-bab8-4f2e52e282ef": "2.02",
            "64fe9abc-6e08-4145-86ed-c23fb2a9003e": "1.50",
            "49fc6e53-1d9d-4dbd-abb6-48b226c1becd": "1.21",
            "d60cb6a9-6ed6-478b-848a-437ad9d472c0": "0.01"
        },
        "labelSuccessRates": {}
    },
    "cards": {
